...
void conversion_string_to_function_code()
{
    ...
    {
        if(check_string(0, "p")){
            uint16_t tirette = PORTAbits.RA9;
            //PutcUART(UART_XBEE, 'k');
            printf("%d",tirette);
            //PutcUART(UART_XBEE, tirette);
        }
        ...
        // Fonction de communication arduino
        else if (check_string(0, "ard"))
        {
            serialus_traitement_arduino(1);
        }//effacer la fenêtre
...
#endif
#ifdef ARDUINO                              //meme si on desactive  serialus avec NO_SERIALUS, son interruption en toujurs active et peut être utilisée pour la communication avec l'arduino
    serialus_traitement_arduino();                //dans ce mode, seul les instaruction de l'arduino sont prises en compte
    
    PutcUART(UART_XBEE,'1');
    delay_ms(1000);
    PutcUART(UART_XBEE,'2');
    delay_ms(1000);
    PutcUART(UART_XBEE,'3');
    delay_ms(1000);
    PutcUART(UART_XBEE,'4');
    delay_ms(1000);
    PutcUART(UART_XBEE,'5'); 
    delay_ms(1000);
    PutcUART(UART_XBEE,'6');
    delay_ms(1000);
    PutcUART(UART_XBEE,'7');
    delay_ms(1000);
    PutcUART(UART_XBEE,'8');
    delay_ms(1000);
    PutcUART(UART_XBEE,'F');
    delay_ms(1000);
    ...
    /*if (serialus.info_a_traiter == true)
    {
        //delay_ms(100);
        if(check_string(0, "0")){
            //uint8_t high[6] = {'h','i','g','h',';',0};
            //PutsUART(UART_XBEE, high, 6);
            PutcUART(UART_XBEE, '1');
        }
        if(check_string(0, "1")){
            //uint8_t low[5]  = {'l','o','w',';',0};
            //PutsUART(UART_XBEE, low, 5);
            PutcUART(UART_XBEE, '0');
        }
        reinit_buffer_serialus();
    }*/
}  
...
/*
 * Fonction qui gère la communication serial avec une arduino
 * Les fonctions içi doivent être envoyer par l'arduino vers la carte
 * @param pos : 0 dans le mode adruino et 1 dans le mode PC (position de la chaine dans la commande envoyée)
 */
void serialus_traitement_arduino()
{
    int ID_module = is_ID_IR();              //l'arduino ne recoit que des ID d'IR (25, 27, 29, 31)
    if(ID_module!=-1){
        
        ramassage_goblet(ID_module);
